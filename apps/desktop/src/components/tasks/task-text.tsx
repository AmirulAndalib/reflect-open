 ...
 * the Tasks view. Inline structure comes from the shared Lezer grammar via
 * {@link scanInlineSegments} — not a regex — so a `[[YYYY-MM-DD]]` link renders
 * as a blue short date (V1's due chip), other `[[wiki]]` links and
 * markdown/bare-URL links render as blue text, and a `[[link]]`/URL inside a
 * code span stays literal, exactly as the editor and indexer see it.
 */
export function TaskText({ task }: { task: OpenTask }): ReactElement {
  const { settings } = useSettings()
  const segments = scanInlineSegments(task.raw.replace(MARKER_RE, ''))
