...
describe('SuggestedContactCard', () => {
  ...
  it('Add writes the details and the `added` mark, then hides the card', async () => {
    renderCard()
    ...
    expect(written[1]?.contents).toContain('contactSuggestion: added')
    await waitFor(() => expect(screen.queryByText('Ada Lovelace')).toBeNull())
  })

  it('Ignore writes only the `ignored` mark, then hides the card', async () => {
    renderCard()
  ...
  it('renders nothing for a note whose suggestion was already resolved', async () => {
    noteSource = '---\ncontactSuggestion: ignored\n---\n# Ada Lovelace\n'
    renderCard()
