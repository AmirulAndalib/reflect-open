...
describe('SettingsScreen', () => {
  ...
  it('toggles full-width notes and persists the preference', async () => {
    renderScreen()
    ...
    await waitFor(() =>
      expect(saved).toEqual([expect.objectContaining({ editorFullWidthNotes: true })]),
    )
  })
  ...
  it('enabling semantic search persists the opt-in', async () => {
    ...
    await waitFor(() =>
      expect(saved).toEqual([
        { editorMarkdownSyntax: 'hide', editorSpellCheck: true, editorDefaultBullet: true, editorBulletAfterHeading: true, editorTextSize: 'small', editorFullWidthNotes: false, semanticSearchEnabled: true, describeAssets: true, contactsEnabled: false, mobileOnboarded: false, mobileStorage: 'local', mobileGraphName: '', theme: 'system', timeFormat: '12h', dateFormat: 'mdy', weekStartDay: 'monday', allNotesFilterTags: ['book', 'link', 'person'], calendarEnabled: false, calendarIds: [], graphColors: {}, aiProviders: [], defaultAiProviderId: null, chatModelSelection: null, aiPrompts: [] },
      ]),
    )
  ...
  it('shows the downloaded model once ready and persists a disable', async () => {
    ...
    await waitFor(() =>
      expect(saved).toEqual([
        { editorMarkdownSyntax: 'hide', editorSpellCheck: true, editorDefaultBullet: true, editorBulletAfterHeading: true, editorTextSize: 'small', editorFullWidthNotes: false, semanticSearchEnabled: false, describeAssets: true, contactsEnabled: false, mobileOnboarded: false, mobileStorage: 'local', mobileGraphName: '', theme: 'system', timeFormat: '12h', dateFormat: 'mdy', weekStartDay: 'monday', allNotesFilterTags: ['book', 'link', 'person'], calendarEnabled: false, calendarIds: [], graphColors: {}, aiProviders: [], defaultAiProviderId: null, chatModelSelection: null, aiPrompts: [] },
      ]),
    )
